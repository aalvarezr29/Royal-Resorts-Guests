...
class vcRestaurant: UIViewController, WKNavigationDelegate, WKUIDelegate {
    ...
    override func viewDidLoad() {
        ...
        if appDelegate.strBundleIdentifier == "com.royalresorts.guestservices"{
            ...
            //Boton Reserve
            ViewItem.rightBarButtonItem = UIBarButtonItem(title: NSLocalizedString("btnReserve",comment:""), style: .plain, target: self, action: #selector(vcRestaurant.clickReserve(_:)))
            
            if self.appDelegate.RestStayInfoID > 0 && self.appDelegate.pkRestaurantID > 0 {
                ViewItem.rightBarButtonItem?.isEnabled = true
            }else{
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        Analytics.logEvent(AnalyticsEventSelectContent, parameters: [
            AnalyticsParameterItemID: "id-Restaurant",
            AnalyticsParameterItemName: "Restaurant",
            AnalyticsParameterContentType: "Pantalla"
            ])
        
        Analytics.setScreenName("Restaurant", screenClass: appDelegate.gstrAppName)
        
    }
    
    @objc func clickReserve(_ sender: AnyObject){
        
        let tercerViewController = self.storyboard?.instantiateViewController(withIdentifier: "vcReservRestAvail") as! vcReservRestAvail
        tercerViewController.RestaurantName = RestaurantName
        tercerViewController.Opentime = Opentime
        tercerViewController.RestaurantCode = RestaurantCode
        tercerViewController.PropertyCode = PropertyCode
        tercerViewController.strDayName = strDayName
    ...
    @objc func GoPagMenu(_ sender: UIButton) {
        //myWebView.loadRequest(NSURLRequest(URL: NSURL(string: urlHome)!))
